...
function faveColorFinder(color){
  if (color === 'red'){
    return 'red is a great color'
      ...
      return 'green is a solid facorite color'
  } else if (color === 'black'){
    return 'so trendy'
  } else {
...
/*
  Write a function that takes in an array of numbers as an argument. In the body of the function, write logic to determine if the array is in ascending order. The function should return true, if it is sorted
   in ascending order, false if it is not. Create a variable, `arrayIsAscending` and set it equal to your function invoked. Use the sample array to test this function.
*/

//CODE HERE

function isItAscending(samepleArray){
  let check;
  for (let i = 0; i < sampleArray.length-1; i++){
    if (samepleArray[i] < sampleArray[i+1] || sampleArray[i+1] === sampleArray[i] )
    {
      check = true
    }
  else
